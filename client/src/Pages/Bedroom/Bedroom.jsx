            ...
            <div className={bedroom.imagetext}>
                ...
                <div className={bedroom.buttons}>
                    {product.variants.map((item) => (
                        <div key={item._id}>
                            <button className={bedroom.currentbutton} onClick={() => setCurrentColor(item)} style={{ backgroundColor: item.color, cursor: 'pointer' }}></button>
                        </div>
                    ))}
                </div>
            </div>
...
const Bedroom = () => {
    ...
    }, [bedroomProducts, sortOption])

    const itemsPerPage = 6
    const totalPages = Math.ceil(bedroomProducts.length / itemsPerPage) || 1
  ...
  return (
    <div className={bedroom.container}>
        <div className={bedroom.main}>
            ...
            <div className={bedroom.pagination}>
                <button disabled={page === 1} onClick={() => setPage(page - 1)}>
                    <MdKeyboardDoubleArrowLeft />
                </button>
                {[...Array(totalPages)].map((_, index) => (
                    <button key={index} onClick={() => setPage(index + 1)} className={page === index + 1 ? bedroom.activepage : ""}>
                        {index + 1}
                    </button>
                ))}
