...
const ProductDetail = () => {
  ...
  useEffect(() => {
    const fetchProduct = async() => {
      try {
          ...
          const filtered = relatedData.filter((item) => item._id.toString() !== foundProduct._id.toString())
  ...
  let availablePages = product?.categories || []
  ...
  return (
    <>
      <div className={productdetail.container}>
        <div className={productdetail.detailleft}>
          ...
          <div className={productdetail.subimage}>
            {product?.variants?.filter((item) => item._id !== selectedVariant._id).slice(0, 2).map((item) => (
                <img key={item._id} src={item.image} alt={product.name} onClick={() => setSelectedVariant(item)} />
              ))}
          </div>
        </div>
        <div className={productdetail.detailright}>
          <div className={productdetail.pagelinks}>
            <Link to={'/'}>Home</Link>/
            <Link to={`/${category || "shopall"}`}>
              {category && category !== "undefined" ? category[0].toUpperCase() + category.slice(1) : "Shopall"}
            </Link>/
            ...
            {product?.variants?.map((item) => (
              <div key={item._id}>
                <button className={productdetail.productcolortype} onClick={() => setSelectedVariant(item)}
                  style={{ backgroundColor: item.color, cursor: 'pointer' }}></button>
              </div>
            ))}
          </div>
          ...
          <div className={productdetail.cartbutton}>
            <div className={productdetail.quantity}>
              ...
              <p>{quantity}</p>
              <button onClick={increase} disabled={!selectedVariant.stock || quantity >= selectedVariant.stock}>+</button>
            </div>
            <div>
              <Props content={!selectedVariant.stock || selectedVariant.stock === 0 ? 'OUT OF STOCK' : 'ADD TO CART'} 
                fsize={'15px'} font={'var(--primary-font)'} bgcolor={'var(--second-color)'} col={'var(--third-color)'} 
                bord={'none'} rad={'0'} pad={'6px 16px'} hbg={'var(--first-color)'} trans={'0.4s'}
                cursor={selectedVariant.stock === 0 ? 'not-allowed' : 'pointer'} oncl={handleAddToCart} 
                ...
                <div>
                  {[1, 2, 3, 4, 5].map((star) => (
                    <span key={star} onClick={() => setRating(star)} onMouseEnter={() => setHover(star)} onMouseLeave={() => setHover(0)}
                      style={{ fontSize: '22px', cursor: 'pointer', color: star <= (hover || rating) ? '#e89c70' : '#ccc' }}>
                      ★
                    </span>
                  ))}
                  {error.rating && (<p style={{ fontSize: '12px', color: 'red' }}>{error.rating}</p>)}
                </div>
              </div>
      ...
      <Relatedproducts relatedProducts={relatedProducts} currentCategory={category} />
    </>
  );
};
